...
### 데이터 / 지표
- [x] **AC1 (스키마 + 신규 필드)**: 통합 `yearDataSchema`에 `inventory`, `restrictedCash` 추가. 한솔 님 상장 PDF 검증 통과.
- [x] **AC2 (상장 유동성 3종)**: 유동비율·당좌비율·현금비율 추가, thresholds.ts 신호등 임계값.
- [x] **AC3 (비상장 신규 4종)**: Runway 4시나리오·BEP 역산·현금 3분류·Burn YoY — 모두 `metrics/private.ts`에 순수함수로.

### 화면 구조
- [x] **AC4 (상장 화면)**: 4섹션 카드(성장/수익/안정/유동) + 밸류에이션 + 동종비교. 중복 제거 완료.
- [x] **AC5 (비상장 화면)**: 8섹션 (안내박스/생존성점수/현금/Burn/Runway/BEP/자본·차입/VC 진단).
- [x] **AC6 (ResultView 분리)**: 1378줄 → `ResultView.tsx` 53줄 + shared/ 6개 + Listed/Private.
- [x] **AC7 (차트=숫자 원칙)**: 매출/마진 차트 옆 KPI 카드 X, 단위 1조 이상 "X.X조" 통일.

### 외부 데이터
- [x] **AC8 (종목코드 자동 추출)**: AI가 PDF에서 stockCode(6자리) + peerSuggestions 추천.
- [x] **AC9 (네이버 금융 스크래핑)**: `/api/peers` — id 기반 엘리먼트(_per/_pbr/_eps/_market_sum) + em 태그 대응.
- [x] **AC10 (밸류에이션 카드)**: PER·PBR·PSR·EPS·BPS·52주·시가총액 표시.
- [x] **AC11 (동종업계 비교)**: 본 회사 + 추천 3개 한 표 비교 (시총·PER·PBR·영업이익률).

### AI / 톤
- [x] **AC12 (AI 분석 강화)**: insight 10문장+, analysis 5문장+ 강제. schema.describe + 프롬프트 양방.
- [x] **AC13 (AI 프롬프트 분기)**: 상장=재무 선배 / 비상장=VC 심사역 15년차. 점수 루브릭 + ACTION 헤더 + VC 질문 3개.

### 스모크
- [x] **AC14 (로컬 스모크)**: 상장 PDF 검증 통과 (SK하이닉스). 비상장·OCR은 한솔 님 검증. `npm run build` ✓ (6.1초, 에러 0).
